...
def cdc_push_add ( changeseq, row, column_names, fieldmap, table, cdc_owner,  p, key, conn,curs, cfg, logger, mandatorydict ) :
   ...
   for k in row :
      ...
      if fieldmap[column_names[columncount]][0] == 'CHAR' and mandatory_to_not_null == False:
         # fwd mode, do nothing
         pass
...
def cdc_track_changes( conn, curs, p, table, keys, tn, seq, cfg, mandatorydict, logger) :
   ...
   if result is False :
      ...
      for row in result :
         ...
         if operation == 1 :
             pushresult = cdc_push_add ( changeseq, row, column_names, fieldmap, table, cdc_owner, p, key, conn, curs, cfg, logger, mandatorydict )

         if operation == 2 :
             pushresult =  cdc_push_delete ( changeseq, row, column_names, fieldmap, table, cdc_owner, p, key, conn, curs, cfg, logger )
         
         if operation == 4 :
             pushresult = cdc_push_update ( changeseq, row, column_names, fieldmap, table, cdc_owner, p, key , conn, curs, cfg, logger)
      ...
      if count == 0 :
         ...
         logger.info('final change sequence is %d initial was %d' % (changeseq,initchgseq))
...
def conn_mariadb(cfg) :
    ...
    try :
       conn = mariadb.connect(host="%s" % host , db="%s" % dbname ,user="%s" % user ,password="%s" % password ,port=port )
...
def conn_oesql(cfg,logger) :
    ...
    conn = jaydebeapi.connect(driverstring,connstring,credentials,dlc)
    logger.info(conn)
...
def extract(cfg,logger,operation) :
   ...
   if process_entire_database :
       logger.info('Extracting all tables except %s from %s/%s' % ( exclude_tables , oedbpath,oedbname))
   ...
   logger.info ('Extracting %s from %s/%s' % ( tablelist,  oedbpath,oedbname))
   ...
   for table in tablelist :
       ...
       if result == False :
           logger.error('Could not extract %s for %s.  Skipping' % (operation,table))
...
def loadData(fn, table, cfg, logger,conn ):
   ...
   if not exists :
     logger.error ('%s DOES NOT exist in %s -- cannot load data' % (table,mdbname))
     # do not exit program, continue on
     return True
   else :
     logger.info('Table %s found in database' % table)

   if require_empty_table == True:
      logger.info('Checking for empty table')
      qry = "SELECT EXISTS ( SELECT %s from %s.%s limit 1)" % (fwd_index_key,mdbname,table)
      cur = run_msql (qry, conn, cfg, logger) 

      if cur == False :
         logger.info('Cannot read row count from %s.%s' % (mdbname,table))
         return False     
      rows = cur.fetchone()
      if rows[0] == True :
   ...
   for token in fields.split(',') :
      try :
         ...
         logger.info('Replacing reserved word %s with %s%s' % (token,token,reserved_suffix))
   ...
   qry =  "LOAD DATA LOCAL INFILE '%s' INTO TABLE %s.%s FIELDS TERMINATED BY '^' ignore 1 lines (%s) set %s = nextval(%s)" % (fn,mdbname,table,fields,fwd_index_key,fwd_sequence)
   # remove last comma
   ...
   if cur == False :
      logger.info('Errors loading dump file for %s.%s' % (mdbname,table))
      if stop_on_error :
         return False     
      else :
   ...
   for line in content :
      ...
      try :
         ...
         try :
            ...
            dummy = reserved_words_dict[t]
            logger.info('Replacing reserved word %s with %s%s' % (tokens[3],tokens[3],reserved_suffix))
         ...
         if tokens[0] == fwd_index_name :
            qrylist.append('ALTER TABLE %s.%s ADD CONSTRAINT %s PRIMARY KEY (%s)' % (mdbname,table,fwd_index_name,tokens[3]))
         else :
            ...
            if tokens[1] == '1' :
               qryline = 'CREATE %s INDEX %s ON %s.%s ( %s ' % ( unique, tokens[0], mdbname, table , tokens[3]) + ','
               indexname = tokens[0]
            else :
...
def loadIndexTextFieldCheck ( qrylist, mdbname, table, conn, cfg, logger) :
   ...
   '''
   check the definition of each field.  if it is a text or mediumtext field, then drop it 
   '''
...
def loadSchema(fn, cfg, logger, conn ):
   ...
   # drop?
   if drop_table_if_exists == True and exists:
   ...
   for line in content :
      ...
      if fieldmandatory == 'yes' :
         if mandatory_to_not_null :
            logger.info('Setting %s to NOT NULL ( mandatory_to_not_null = True in cfg )' % (fieldname))
            nullable = ' NOT NULL '
...
def loadSequence(cfg,logger,conn) :
   ...
   for line in content :
       ...
       qrylst.append(qry)
       qry = 'CREATE SEQUENCE %s INCREMENT BY 1 MINVALUE %s NOMAXVALUE START WITH %s CYCLE' % ( seqname,seqmin,curval )
       qrylst.append(qry)
...
def mapformat(fieldtype,fieldformat,fieldwidth,cfg,logger) :
   ...
   if fieldtype == 'varchar' :
      try :
         ...
         logger.error('Cannot determine field format for %s' % fieldtype)
         return False
   else :
      fieldformat = ''
...
def postprocessData(mdbname,table,cfg,conn,logger) :
    ...
    for file in files :
       ...
       try :
          ...
          qry = "UPDATE %s.%s SET %s = '%s' WHERE %s = '%s'" % ( mdbname, table, fieldname, clob, fieldname, file )
          qrylist.append(qry)
       except Exception as e :
          logger.error(e)
          return False
    ...
    for line in schema :
        ...
        if tokens[1] in extent_special_dict :
            try :
                ...
                logger.error('Error handling special extent field %s' % tokens[1])
                return False
            ...
            # handles timestamep and datetime
...
def runabl(query,cfg,logger):
    ...
    try :
       ...
       runstring = 'TERM=ansi;export $TERM;_progres -db %s/%s' % (oedbpath,oedbname)
       runstring = runstring + ' -p ' + tf.name + ' ' + pfparams 
       logger.debug (runstring)

       oecmd = subprocess.Popen(runstring,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
