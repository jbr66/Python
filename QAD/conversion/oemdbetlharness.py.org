...
python3.7 ./oemdbetlharness.py --cfg oemdbetl.yaml --loglevel INFO --process_functions loadSchema --process_entire_database True
python3.7 ./oemdbetlharness.py --cfg oemdbetl.yaml --loglevel DEBUG --process_functions loadData --include_tables Document
python3.7 ./oemdbetlharness.py --cfg oemdbetl.yaml --loglevel info --process_functions extractIndex

Note that you can run multiple versions of this in parallel by using include_tables pointing to distinct subsets of tables
...
if __name__ == '__main__' :
   ...
   if cfg['process_entire_database']:
       ...
       print ('On the following tables in database %s/%s: %s'  % (cfg['oedbpath'],cfg['oedbname'],cfg['include_tables']))
   print ('The following tables are excluded : %s' % ( cfg['exclude_tables']))

   print ('Refer to %s for the etl logs, and %s for the etl error messages' % (cfg['logfile'],cfg['logerror']))
   ...
   if cfg['create_mdb']:
       ...
       tmp_db = cfg['mdbname']
       cfg['mdbname'] = ''
       conn = qadoemdbetl.conn_mariadb(cfg)
       if conn == False :
           print ('Because we are trying to create the databases we cannot connect to MariaDB, the conversion process is prematurely exiting.')
   ...
   if conn == True :
       ...
       conn = qadoemdbetl.conn_mariadb(cfg)
       if conn == False :
           print ('Because we are Loading data and we cannot connect to MariaDB, the conversion process is prematurely exiting.')
           elapsed_time(start_time)
           sys.exit(2)
